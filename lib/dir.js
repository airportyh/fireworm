...
function Dir(dirpath, sink, skipDirEntryPatterns){
  assert(is.string(dirpath))
  assert(skipDirEntryPatterns == null || is.array(skipDirEntryPatterns))
  assert(sink)
  ...
  _update: function(doneUpdate){
    ...
    fs.readdir(this.path, function(err, entryNames){
      ...
      async.each(newEntries, function(entryName, next){
        if (self.skipDirEntryPatterns){
          if (self.skipDirEntryPatterns.some(function(pattern){
