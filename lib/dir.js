...
function Dir(dirpath, sink, initial, options){
...
Dir.prototype = {
  _update: function(){
    var args = getArgs('[initial]:boolean, [doneUpdate]:function', arguments)
    var doneUpdate = args.doneUpdate || function(){}
      ...
      async.each(newEntries, function(entryName, next){
        if (self.options.skipDirEntryPatterns){
          if (self.options.skipDirEntryPatterns.some(function(pattern){
            return entryName.match(pattern)
          })) return next()
        }
        ...
        fs.stat(entryPath, function(err, stat){
          ...
          if (stat.isDirectory()){
            var dir = self.entries[entryName] = new Dir(
              entryPath, self.sink, initial, self.options)
            dir.update(initial, next)
          }else{
            if (!(entryName in self.entries)){
              self.entries[entryName] = new File(
                entryPath, stat, self.sink, initial, self.options)
            }
            next(null)
          }
        })
